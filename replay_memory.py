...
class ReplayMemory:
    def __init__(self, config):
        ...
        self.screens = np.empty(
            (self.config.mem_size, self.config.screen_height, self.config.screen_width),
            dtype=np.uint8,
        )
...
class DQNReplayMemory(ReplayMemory):
    ...
    def getState(self, index):
        ...
        if index >= self.config.history_len - 1:
            ...
            indices = [
                (index - i) % self.count
                for i in reversed(range(self.config.history_len))
            ]
    ...
    def sample_batch(self):
        ...
        while len(indices) < self.config.batch_size:
            ...
            while True:
                ...
                if (
                    index >= self.current
                    and index - self.config.history_len < self.current
                ):
                    continue
...
class DRQNReplayMemory(ReplayMemory):
    def __init__(self, config):
        ...
        self.states = np.empty(
            (
                self.config.batch_size,
                self.config.min_history + self.config.states_to_update + 1,
                self.config.screen_height,
                self.config.screen_width,
            ),
            dtype=np.uint8,
    ...
    def getState(self, index):
        a = self.screens[
            index - (self.config.min_history + self.config.states_to_update + 1) : index
        ]
        return a
        ...
        while len(indices) < self.config.batch_size:
            ...
            while True:
                index = random.randint(
                    10, self.count - 1
                )  # (self.config.min_history, self.count-1)
            ...
            (
                self.actions_out[len(indices)],
                self.terminals_out[len(indices)],
                self.rewards_out[len(indices)],
            ) = self.get_scalars(index)
            indices.append(index)
