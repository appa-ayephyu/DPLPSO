...
class DQNAgent(BaseAgent):
    ...
    def observe(self):
        ...
        self.replay_memory.add(
            screen, reward, self.env_wrapper.action, self.env_wrapper.terminal
        )
        ...
        if self.i % self.config.train_freq == 0 and self.i > self.config.train_start:
            ...
            q, loss = self.net.train_on_batch_target(
                state, action, reward, state_, terminal, self.i
            )
            self.total_q += q
    ...
    def policy(self):
        if np.random.rand() < self.epsilon:
            ...
            a = self.net.q_action.eval({self.net.state: [state]}, session=self.net.sess)
            return a[0]
    ...
    def play(self, episodes, net_path):
        ...
        while i < episodes:
            a = self.net.q_action.eval(
                {self.net.state: [self.history.get() / 255.0]}, session=self.net.sess
            )
            action = a[0]
