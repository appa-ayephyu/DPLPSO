...
r"""
A Discrete Particle Swarm Optimization (discrete PSO) algorithm.
...
is often applied to discrete problems such as job-shop scheduling,
...
In addition, one notable change for dicrete PSO is that the position
update rule is now decided upon by the ceiling of position + velocity


This enables the algorithm to output discrete positions rather than
...
class DiscretePSO(DiscreteSwarmOptimizer):
    ...
    def _compute_position(self, swarm):
        ...
        This computes the next position in a discrete swarm. It compares the
        ...
        position = math.ceil(swarm.poisition + swarm.velocity)
        if position < 0:
            position = 0
        elif position > swarm.dimensions:
            position = swarm.dimensions
