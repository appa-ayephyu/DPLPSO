...
class BinaryPSO(DiscreteSwarmOptimizer):
    ...
    def _compute_position(self, swarm):
        ...
        position = [x[0] for x in swarm.position]
        velocity = [v[0] for v in swarm.velocity]
        position = [round(position[n] + velocity[n]) for n in range(swarm.n_particles)]
        position = [0 if x < 0 else x for x in position]
        position = [
            swarm.dimensions - 1 if x > swarm.dimensions - 1 else x for x in position
        ]

        return_pos = swarm.position.copy()
        for n in range(swarm.n_particles):
            return_pos[n][0] = position[n]
