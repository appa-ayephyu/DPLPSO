...
from utils import conv2d_layer, fully_connected_layer, stateful_lstm, huber_loss
...
class DRQN(BaseModel):
    ...
    def add_placeholders(self):
        ...
        self.state_target = tf.placeholder(
            tf.float32,
            shape=[None, 1, self.screen_height, self.screen_width],
            name="input_target",
        )
        # create placeholder to fill in lstm state
        self.c_state_train = tf.placeholder(
            tf.float32, [None, self.lstm_size], name="train_c"
        )
        self.h_state_train = tf.placeholder(
            tf.float32, [None, self.lstm_size], name="train_h"
        )
        self.lstm_state_train = tf.nn.rnn_cell.LSTMStateTuple(
            self.c_state_train, self.h_state_train
        )

        self.c_state_target = tf.placeholder(
            tf.float32, [None, self.lstm_size], name="target_c"
        )
        self.h_state_target = tf.placeholder(
            tf.float32, [None, self.lstm_size], name="target_h"
        )
        self.lstm_state_target = tf.nn.rnn_cell.LSTMStateTuple(
            self.c_state_target, self.h_state_target
        )
        ...
        self.initial_zero_complete = np.zeros(
            (self.num_lstm_layers, 2, self.batch_size, self.lstm_size)
        )

        self.dropout = tf.placeholder(dtype=tf.float32, shape=[], name="dropout")
        self.lr = tf.placeholder(dtype=tf.float32, shape=[], name="lr")
        self.target_val = tf.placeholder(
            dtype=tf.float32, shape=[None], name="target_val"
        )
        self.terminal = tf.placeholder(dtype=tf.float32, shape=[None], name="terminal")
        self.target_val_tf = tf.placeholder(
            dtype=tf.float32, shape=[None, self.n_actions]
        )
    ...
    def add_logits_op_train(self):
        ...
        w, b, out = fully_connected_layer(
            out, self.n_actions, scope_name="out_train", activation=None
        )
    ...
    def add_logits_op_target(self):
        ...
        w, b, out = fully_connected_layer(
            out, self.n_actions, scope_name="out_target", activation=None
        )
    ...
    def train_on_batch_target(self, states, action, reward, terminal, steps):
        ...
        for i in range(self.min_history):
            ...
            (
                ...
                [
                    self.state_output_c,
                    self.state_output_h,
                    self.state_output_target_c,
                    self.state_output_target_h,
                ],
                {
                    self.state: states[i],
        ...
        for i in range(self.min_history, self.min_history + self.states_to_update):
            ...
            target = (1.0 - terminal[i]) * self.gamma * max_target + reward[i]
    ...
    def add_loss_op_target(self):
        action_one_hot = tf.one_hot(
            self.action, self.n_actions, 1.0, 0.0, name="action_one_hot"
        )
        train = tf.reduce_sum(
            self.q_out * action_one_hot, reduction_indices=1, name="q_acted"
        )
    ...
    def update_target(self):
        for name in self.w:
            self.target_w_assign[name].eval(
                {self.target_w_in[name]: self.w[name].eval(session=self.sess)},
                session=self.sess,
            )
        for var in self.lstm_vars:
            self.target_w_assign[var.name].eval(
                {self.target_w_in[var.name]: var.eval(session=self.sess)},
                session=self.sess,
            )
    ...
    def init_update(self):
        ...
        for name in self.w:
            self.target_w_in[name] = tf.placeholder(
                tf.float32, self.w_target[name].get_shape().as_list(), name=name
            )
            self.target_w_assign[name] = self.w_target[name].assign(
                self.target_w_in[name]
            )

        self.lstm_vars = tf.get_collection(
            tf.GraphKeys.TRAINABLE_VARIABLES, scope="lstm_train"
        )
        lstm_target_vars = tf.get_collection(
            tf.GraphKeys.TRAINABLE_VARIABLES, scope="lstm_target"
        )

        for i, var in enumerate(self.lstm_vars):
            self.target_w_in[var.name] = tf.placeholder(
                tf.float32, var.get_shape().as_list()
            )
            self.target_w_assign[var.name] = lstm_target_vars[i].assign(
                self.target_w_in[var.name]
            )
