...
from utils import conv2d_layer, fully_connected_layer, huber_loss
...
class DQN(BaseModel):
    ...
    def train_on_batch_target(self, state, action, reward, state_, terminal, steps):
        state_ = state_ / 255.0
        state = state / 255.0
        target_val = self.q_target_out.eval(
            {self.state_target: state_}, session=self.sess
        )
        max_target = np.max(target_val, axis=1)
        target = (1.0 - terminal) * self.gamma * max_target + reward
